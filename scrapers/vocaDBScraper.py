...
class VocaDBScraper:
    ...
    async def gen_urls(self, start: int = 0, size: int = 100):
        ...
        urls = [
            self.gen_url(i * 100, 100) for i in range(total_pages + 1)
        ]
        return urls
    ...
    async def process_url(self, session: aiohttp.ClientSession, url: str, pbar: tqdm):
        async with self.sem:
            try:
    ...
    async def run(self):
        ...
        async with aiohttp.ClientSession() as session:
            with tqdm(total=len(urls), desc="Fetching songs") as pbar:
                tasks = [
